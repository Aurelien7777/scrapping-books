...
for urls in all_urls_category_sans_index:
    ...
    print(f"Création d'un dossier image pour la catérogie : {all_category_name[compteur]}")
                ...
                if response.ok: #Vérification de la connexion à la page
                    ...
                    for book in all_book_on_page: 
                        ...
                        if response_book.ok:
                            ...
                            #os.rename(filename, f"image_{book_name}.jpg") 
...
"""for image in list_all_images:
    filename = os.path.basename(image) #"filename" extrait le nom du fichier depuis l’URL avec os.path.basename(image).
    with urllib.request.urlopen(image) as url: #Téléchargement de l'image
        image_data = url.read() #Lit les données de l’image.
    # Enregistre le contenu de l'image dans un fichier
    with open(filename, "wb") as f: #Crée un fichier local avec le nom extrait.
        f.write(image_data) #Écrit les données de l’image dans ce fichier en mode binaire ("wb")."""
